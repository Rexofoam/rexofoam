                    ...
                    <div className="grid grid-cols-5 gap-2 max-w-md mx-auto">
                      {Array.from({ length: 30 }, (_, index) => {
                        ...
                        if (index === 1 || index === 3 || index === 8 || index === 25 || index === 26) {
                          ...
                          // Hat slot: 2
                          const hats = characterData.itemEquipment?.item_equipment?.filter(
                            item => item.item_equipment_part === 'HAT'
                          ) || [];
                          item = hats[0] || null;
                        } else if (index === 4) {
                          // Emblem slot: 4
                          const emblems = characterData.itemEquipment?.item_equipment?.filter(
                            item => item.item_equipment_part === 'EMBLEM'
                          ) || [];
                          item = emblems[0] || null;
                        } else if (index === 7) {
                          // Face Accessory slot: 7
                          const faceAccessories = characterData.itemEquipment?.item_equipment?.filter(
                            item => item.item_equipment_part === 'FACE ACCESSORY'
                          ) || [];
                          item = faceAccessories[0] || null;
                        } else if (index === 9) {
                          // Badge slot: 9
                          const badges = characterData.itemEquipment?.item_equipment?.filter(
                            item => item.item_equipment_part === 'Badge'
                          ) || [];
                          item = badges[0] || null;
                        } else if (index === 12) {
                          // Eye Accessory slot: 12
                          const eyeAccessories = characterData.itemEquipment?.item_equipment?.filter(
                            item => item.item_equipment_part === 'EYE ACCESSORY'
                          ) || [];
                          item = eyeAccessories[0] || null;
                        } else if (index === 13) {
                          // Earring slot: 13
                          const earrings = characterData.itemEquipment?.item_equipment?.filter(
                            item => item.item_equipment_part === 'EARRING'
                          ) || [];
                          item = earrings[0] || null;
                        } else if (index === 14) {
                          // Medal slot: 14
                          const medals = characterData.itemEquipment?.item_equipment?.filter(
                            item => item.item_equipment_part === 'MEDAL'
                          ) || [];
                          item = medals[0] || null;
                        } else if (index === 17) {
                          // Top slot: 17
                          const tops = characterData.itemEquipment?.item_equipment?.filter(
                            item => item.item_equipment_part === 'TOP'
                          ) || [];
                          item = tops[0] || null;
                        } else if (index === 18) {
                          // Shoulder Decoration slot: 18
                          const shoulderDecorations = characterData.itemEquipment?.item_equipment?.filter(
                            item => item.item_equipment_part === 'SHOULDER DECORATION'
                          ) || [];
                          item = shoulderDecorations[0] || null;
                        } else if (index === 19) {
                          // Secondary Weapons slot: 19
                          const secondaryWeapons = characterData.itemEquipment?.item_equipment?.filter(
                            item => item.item_equipment_slot === 'Secondary Weapons'
                          ) || [];
                          item = secondaryWeapons[0] || null;
                        } else if (index === 0 || index === 5 || index === 10 || index === 15) {
                          // Ring slots: 0, 5, 10, 15
                          const rings = characterData.itemEquipment?.item_equipment?.filter(
                          ...
                          // Pendant slots: 6, 11
                          const pendants = characterData.itemEquipment?.item_equipment?.filter(
                          ...
                          // Weapon slot: 16
                          const weapons = characterData.itemEquipment?.item_equipment?.filter(
                            item => item.item_equipment_slot === 'WEAPON'
                          ) || [];
                          item = weapons[0] || null;
                        } else if (index === 20) {
                          // Pocket Item slot: 20
                          const pocketItems = characterData.itemEquipment?.item_equipment?.filter(
                            item => item.item_equipment_part === 'Pocket Item'
                          ) || [];
                          item = pocketItems[0] || null;
                        } else if (index === 21) {
                          // Belt slot: 21
                          const belts = characterData.itemEquipment?.item_equipment?.filter(
                            item => item.item_equipment_part === 'BELT'
                          ) || [];
                          item = belts[0] || null;
                        } else if (index === 22) {
                          // Bottom slot: 22
                          const bottoms = characterData.itemEquipment?.item_equipment?.filter(
                            item => item.item_equipment_part === 'BOTTOM'
                          ) || [];
                          item = bottoms[0] || null;
                        } else if (index === 23) {
                          // Glove slot: 23
                          const gloves = characterData.itemEquipment?.item_equipment?.filter(
                            item => item.item_equipment_part === 'GLOVE'
                          ) || [];
                          item = gloves[0] || null;
                        } else if (index === 24) {
                          // Cape slot: 24
                          const capes = characterData.itemEquipment?.item_equipment?.filter(
                            item => item.item_equipment_part === 'CAPE'
                          ) || [];
                          item = capes[0] || null;
                        } else if (index === 27) {
                          // Shoes slot: 27
                          const shoes = characterData.itemEquipment?.item_equipment?.filter(
                            item => item.item_equipment_part === 'SHOES'
                          ) || [];
                          item = shoes[0] || null;
                        } else if (index === 29) {
                          // Mechanical Heart slot: 29
                          const mechanicalHearts = characterData.itemEquipment?.item_equipment?.filter(
                            item => item.item_equipment_part === 'Mechanical Heart'
                          ) || [];
                          item = mechanicalHearts[0] || null;
                        } else {
                          // Other slots: remaining equipment
                          const nonSpecialItems = characterData.itemEquipment?.item_equipment?.filter(
                            item => item.item_equipment_part !== 'RING' && 
                        ...
                        return (
                          <div 
                            ...
                            title={`${item.item_equipment_part}: ${item.item_name}${item.starforce ? ` (★${item.starforce})` : ''}${item.potential_option_grade ? ` [${item.potential_option_grade}]` : ''}`}
                          >
                          {item.item_icon ? (
                            <img
                              src={item.item_icon}
                          ...
                          {(() => {
                            ...
                            return (
                              <div className={`absolute top-1 left-1 ${rarityColor} text-white text-xs w-4 h-4 rounded-full flex items-center justify-center font-bold`}>
                                {rarityLetter}
                              </div>
                            );
                          })()}
                          ...
                          <div className="absolute bottom-full left-1/2 transform -translate-x-1/2 mb-2 px-3 py-2 bg-black text-white text-xs rounded opacity-0 group-hover:opacity-100 transition-opacity pointer-events-none z-10 min-w-max">
                            <div className="whitespace-nowrap">
                              <div className="font-semibold">
                                {item.potential_option_grade ? `(${item.potential_option_grade}) ` : '(Common) '}{item.item_name}
                              </div>
                              <div className="text-gray-300 mb-1">{item.item_equipment_part}</div>
                              
                              {/* Item Potential */}
                              {(item.potential_option_1 || item.potential_option_2 || item.potential_option_3) && (
                                <div className="mt-2">
                                  <div className="text-blue-300 font-semibold">Item Potential:</div>
                                  {item.potential_option_1 && <div className="text-green-300">• {item.potential_option_1}</div>}
                                  {item.potential_option_2 && <div className="text-green-300">• {item.potential_option_2}</div>}
                                  {item.potential_option_3 && <div className="text-green-300">• {item.potential_option_3}</div>}
                                </div>
                              )}
                              ...
                              {(item.additional_potential_option_1 || item.additional_potential_option_2 || item.additional_potential_option_3) && (
                                <div className="mt-2">
                                  <div className="text-purple-300 font-semibold">Additional Potential:</div>
                                  {item.additional_potential_option_1 && <div className="text-yellow-300">• {item.additional_potential_option_1}</div>}
                                  {item.additional_potential_option_2 && <div className="text-yellow-300">• {item.additional_potential_option_2}</div>}
                                  {item.additional_potential_option_3 && <div className="text-yellow-300">• {item.additional_potential_option_3}</div>}
                                </div>
                              )}
                              
                              {/* Starforce info */}
                              {item.starforce && (
                                <div className="mt-1 text-yellow-300">Starforce: ★{item.starforce}</div>
                              )}
                            </div>
                          </div>
                        </div>
